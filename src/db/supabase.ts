...
export async function fetchFailedRawNews(limit: number, sinceHours: number): Promise<PendingRawNewsRecord[]> {
	const supabase = getSupabaseClient();
	...
	if (error) {
		throw new Error(`Failed to fetch failed raw_news: ${error.message}`);
	}

	return (data ?? []) as PendingRawNewsRecord[];
}

export async function tryMarkRawNewsAsProcessing(
	newsId: string,
	fromStatus: RawNewsStatus = "PENDING",
): Promise<boolean> {
	const supabase = getSupabaseClient();
