...
import { processDailySummary, processFailedNewsAnalysis, processNewsAnalysis } from "../analysis/service";
import { config } from "../config";
import { scrapeAndUpload } from "../scrape/service";

type ScrapeSource = "manual" | "cron";
type AnalysisSource = "manual" | "cron" | "retry_manual";
type AnalysisMode = "pending" | "failed";

type FailedRetryOptions = {
	limit: number;
	sinceHours: number;
};
...
export const createJobController = () => {
	...
	const runAnalysisJob = async (
		...
		startedAtMs: number,
		mode: AnalysisMode,
		retryOptions?: FailedRetryOptions,
	): Promise<void> => {
		try {
			const analysis = mode === "failed" ? await processFailedNewsAnalysis(retryOptions) : await processNewsAnalysis();
			const summary = await processDailySummary();
			const durationMs = Date.now() - startedAtMs;
			console.log(
				`[${new Date().toISOString()}] Analysis completed (source=${source}, mode=${mode}, durationMs=${durationMs}, processed=${analysis.processed}, succeeded=${analysis.succeeded}, failed=${analysis.failed}, summaryDate=${summary.summaryDate}, analyzedCount=${summary.analyzedCount})`,
			);
		} catch (error) {
			const durationMs = Date.now() - startedAtMs;
			console.error(
				`[${new Date().toISOString()}] Analysis failed (source=${source}, mode=${mode}, durationMs=${durationMs})`,
				error,
			);
		} finally {
			const finishedAt = new Date().toISOString();
	...
	const triggerAnalysis = (source: AnalysisSource): TriggerResult => {
		...
		runAnalysisJob(source, startedAtMs, "pending");

		return { started: true };
	};

	const triggerFailedAnalysisRetry = (source: AnalysisSource, options: FailedRetryOptions): TriggerResult => {
		if (isAnalysisRunning) {
			return {
				started: false,
				reason: "already_running",
		...
		console.log(
			`[${analysisStartedAtIso}] Analysis retry lock acquired (source=${source}, limit=${options.limit}, sinceHours=${options.sinceHours})`,
		);

		runAnalysisJob(source, startedAtMs, "failed", options);
