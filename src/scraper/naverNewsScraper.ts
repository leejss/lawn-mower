...
const scrapeFromPage = async (page: Page): Promise<Article> => {
	...
	const body = await getFirstText(
		page,
		["#dic_area", "article#dic_area", ".go_trans._article_content"],
		true,
	);

	const publishedAt = await getPublishedAt(page);
	const fallbackTitle =
		title.length > 0 ? title : normalizeSingleLine(await page.title());
...
const scrapeWithContext = async (
	...
	try {
		...
		await page
			.waitForLoadState("networkidle", { timeout: WAIT_TIMEOUT_MS })
			.catch(() => undefined);
