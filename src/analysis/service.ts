...
const FAILED_RETRY_LOOKBACK_HOURS = 24;

type AnalysisSourceStatus = "PENDING" | "FAILED";
...
const processAnalysisBatch = async ({
	...
	const pendingItems =
		sourceStatus === "FAILED"
			? await fetchFailedRawNews(limit, sinceHours ?? FAILED_RETRY_LOOKBACK_HOURS)
			: await fetchPendingRawNews(limit);
	...
	for (const item of pendingItems) {
		const locked = await tryMarkRawNewsAsProcessing(item.news_id, sourceStatus);
		if (!locked) {
			continue;
		}
		...
		try {
			...
			failed += 1;
			console.error(`[analysis] ai request failed news_id=${item.news_id}`, error);
			throw error;
		}
...
export async function processFailedNewsAnalysis({
	limit = config.analysisBatchSize,
	sinceHours = FAILED_RETRY_LOOKBACK_HOURS,
}: {
	limit?: number;
	sinceHours?: number;
} = {}): Promise<{
