...
const assertEnv = (): void => {
  if (!ANALYSIS_TABLE_NAME || !RAW_NEWS_TABLE_NAME) {
    throw new Error("ANALYSIS_TABLE_NAME 또는 RAW_NEWS_TABLE_NAME 환경변수가 없습니다.");
  }
};
...
const toRawNewsItem = (record: DynamoDBRecord): RawNewsItem | null => {
  ...
  const item = unmarshall(image) as RawNewsItem;
...
const extractKeywords = (title: string, body: string): string[] => {
  const stopwords = new Set(["그리고", "하지만", "대한", "있는", "한다", "했다", "으로", "에서", "하는"]);
...
const classifySentiment = (body: string): "POSITIVE" | "NEGATIVE" | "NEUTRAL" => {
...
const markFailure = async (item: RawNewsItem, errorMessage: string): Promise<void> => {
  await ddbClient.send(
    new UpdateCommand({
      TableName: RAW_NEWS_TABLE_NAME,
      Key: { news_id: item.news_id },
